/**
 * Script per convertir els articles del Llibre Sisè en entrades de coneixement per al RAG.
 *
 * Ús:
...
const SOURCE_PATH = path.join(
  __dirname,
  '../data/chapters/extracted-articles-llibre-sise.ts'
);
const OUTPUT_PATH = path.join(__dirname, '../data/rag/llibre-sise.json');
...
function main() {
  ...
  if (!Array.isArray(articles)) {
    console.error('❌ No s’han pogut obtenir els articles del Llibre Sisè.');
    process.exit(1);
  }
...
function executeModule(sanitizedCode) {
  ...
  try {
    const script = new vm.Script(sanitizedCode, {
      filename: 'llibre-sise-articles.js'
    });
...
function toKnowledgeEntry(article) {
  const section = article.section || 'Llibre sisè';
...
function normalizeId(number) {
  return `LVI_${number.replace(/\s+/g, '_').replace(/[^\w\-À-ÿ]/g, '')}`;
}
