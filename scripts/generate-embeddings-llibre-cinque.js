/**
 * Script per generar embeddings del corpus del Llibre Cinquè.
 *
 * Ús:
 ...
 *   - Arxiu data/rag/llibre-cinque.json amb les entrades de coneixement.
 */

const fs = require('fs');
const path = require('path');

const KNOWLEDGE_PATH = path.join(__dirname, '../data/rag/llibre-cinque.json');
const OUTPUT_PATH = path.join(
  __dirname,
  '../data/rag/llibre-cinque-embeddings.json'
);
...
async function main() {
  if (!OPENAI_API_KEY) {
    console.error(
      '❌ Fes servir la variable d\'entorn OPENAI_API_KEY abans d\'executar el script.'
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(KNOWLEDGE_PATH)) {
    console.error(
      `❌ No s'ha trobat l'arxiu de coneixement: ${KNOWLEDGE_PATH}`
    );
    process.exit(1);
  }
