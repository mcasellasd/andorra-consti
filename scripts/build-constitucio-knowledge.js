/**
 * Script per generar el fitxer JSON de coneixement de la Constitució d'Andorra
 * a partir dels articles disponibles
 *
 * Ús:
 *   node scripts/build-constitucio-knowledge.js
 */

const fs = require('fs');
const path = require('path');

const ARTICLES_PATH = path.join(__dirname, '../data/codis/constitucio/articles-template.ts');
const OUTPUT_PATH = path.join(__dirname, '../data/rag/constitucio.json');

// Llegir els articles des del fitxer TypeScript
// Com que és un fitxer TS, necessitem una aproximació diferent
// Per ara, crearem les entrades manualment basades en els articles coneguts

function buildKnowledgeEntries() {
  // Entrades basades en el text complet de la Constitució d'Andorra
  const entries = [
    {
      id: "CONST_PREAMB",
      category: "Preàmbul",
    ...
    },
    {
      ...
      keyConcepts: ["norma suprema", "ordenament jurídic", "principis de legalitat", "seguretat jurídica", "dret internacional", "tractats internacionals", "no retroactivitat"]
    }
  ];
...
function main() {
  console.log('📚 Generant fitxer de coneixement de la Constitució d\'Andorra...');
  
  const entries = buildKnowledgeEntries();
  
  console.log(`✅ Generades ${entries.length} entrades de coneixement`);
