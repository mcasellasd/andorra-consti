/**
 * Script per generar embeddings per a tots els documents de legislació andorrana
 * 
 * Ús:
 *   node scripts/generate-embeddings-legislacio.js [norma-id]
 * 
 * Requisits:
 *   - Variable d'entorn OPENAI_API_KEY establerta
 *   - Fitxers JSON de RAG a data/rag/legislacio-andorrana/
 */
...
async function main() {
  ...
  const filesToProcess = normaId
    ? files.filter(f => f === `${normaId}.json`)
    : files;
