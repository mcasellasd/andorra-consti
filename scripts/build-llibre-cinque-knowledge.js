/**
 * Script per convertir els articles del Llibre Cinquè en entrades de coneixement per al RAG.
 *
 * Ús:
 *   node scripts/build-llibre-cinque-knowledge.js
 */
...
const SOURCE_PATH = path.join(
  __dirname,
  '../data/chapters/extracted-articles.ts'
);
const OUTPUT_PATH = path.join(__dirname, '../data/rag/llibre-cinque.json');
...
function main() {
  ...
  if (!Array.isArray(articles)) {
    console.error('❌ No s’han pogut obtenir els articles del Llibre Cinquè.');
    process.exit(1);
  }
...
function sanitizeSource(code) {
  return (
    code
      // elimina importacions
      .replace(/import\s+\{[^}]+\}\s+from\s+['"][^'"]+['"];?\s*/g, '')
      // substitueix exportació
      .replace(
...
function executeModule(sanitizedCode) {
  ...
  try {
    const script = new vm.Script(sanitizedCode, {
      filename: 'llibre-cinque-articles.js'
    });
...
function toKnowledgeEntry(article) {
  const section = article.section || 'Llibre cinquè';
...
function normalizeId(number) {
  return `LV_${number.replace(/\s+/g, '_').replace(/[^\w\-À-ÿ]/g, '')}`;
}
