...
function mapQueryResult(result: QueryResult<RagMetadata>): RetrievedContext | null {
  ...
  const bookId = metadata.sourceType === 'historia_constitucional'
    ? 'HISTORIA_CONSTITUCIONAL'
    : metadata.sourceType === 'doctrina' ? 'DOCTRINA' : 'CONSTITUCIO';
  return {
    bookId,
    score: result.score,
    entry: {
      ...
      category: metadata.category || (
        bookId === 'HISTORIA_CONSTITUCIONAL'
          ? 'Història constitucional'
          : bookId === 'DOCTRINA' ? 'Doctrina' : 'Constitució'
      ),
...
export async function retrieveHistoricalMatches(
  queryText: string,
  topK = 3,
): Promise<RetrievedContext[]> {
  if (!queryText.trim()) return [];
  ...
  const results = await getVectorIndex().query({
    data: queryText,
    topK,
    filter: "sourceType = 'historia_constitucional'",
    includeData: true,
    includeMetadata: true,
    queryMode: QueryMode.HYBRID,
    fusionAlgorithm: FusionAlgorithm.RRF,
